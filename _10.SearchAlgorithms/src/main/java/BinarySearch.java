public class BinarySearch {
    ...
    public static int binarySearchIterative(int[] array, int data){
        ...
        while (left<=right){
            ...
            if (data<array[middle]) right= middle-1; //change right pointer
            else left=middle+1; //change left pointer
        }
        return-1;
    }
    ...
    public static int binarySearchRecursive (int[] array, int data, int left, int right){
        if (left>right) return-1; //couldnt find the element
        int middle=(left+right)/2;
        if (array[middle]==data) return middle;
        if (data<array[middle]) return binarySearchRecursive(array, data,left, middle-1);//change right pointer
        else return binarySearchRecursive(array, data,middle+1, right);//change left pointer
    }
}
