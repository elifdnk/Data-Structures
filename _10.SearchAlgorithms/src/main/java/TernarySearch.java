public class TernarySearch {
    ...
    public static int ternarySearchRecursive(int[] array, int data, int left, int right){
        ...
        if (array[middle1] == data) return middle1;
        if (array[middle2] == data) return middle2;
        if (data<array[middle1]) return ternarySearchRecursive(array,data, left, middle1-1);//changed left
        if(data>array[middle2]) return ternarySearchRecursive(array, data, middle2+1, right);//changed right
        else return ternarySearchRecursive(array, data, middle1+1, middle2-1);//left and right changed
